...
global.zoommeeting = zoommeeting;
...
function showWaitingWindow() {
  if (!waitingWindow)
  {
    ...
    waitingWindow = new BrowserWindow({ width: 1300, height: 800,
      webPreferences: {
    ...
    waitingWindow.webContents.openDevTools({
      mode: 'bottom'
  })
  }
  if (mainWindow){
...
function showInMeetingWindow() {
  if (!inmeetingWindow)
  {
    ...
    inmeetingWindow.webContents.openDevTools({
      mode: 'bottom'
  })
  }
  if (mainWindow){
...
function showYUVWindow() {
  if (!YUVWindow) {
    ...
    YUVWindow.webContents.openDevTools({
      mode: 'bottom'
  })
  }
  if (mainWindow){
...
function showDomainwindow(){
  if (!domainWindow)
  {
    ...
    domainWindow.webContents.openDevTools({
      mode: 'bottom'
  })
  }
  if (mainWindow) {
...
function showLoginWindow(){
  ...
  if (!loginWindow)
  {
    loginWindow = new BrowserWindow({ width: 1300, height: 800,
      webPreferences: {
        nodeIntegration: true
      }  
    });
...
function showStartJoinWindow(){
  if (!startjoinWindow)
  {
    startjoinWindow = new BrowserWindow({ width: 1300, height: 800,
      webPreferences: {
        nodeIntegration: true
      } 
    });
    ...
    startjoinWindow.webContents.openDevTools({
      mode: 'bottom'
  })
  }
  if (mainWindow){
...
let functionObj = {
  showSatrtJoinUnLoginWindow: function() {
    if (!startjoinUnLoginWindow) {
      startjoinUnLoginWindow = new BrowserWindow({ width: 1300, height: 800,
        webPreferences: {
          nodeIntegration: true
        } 
      });
      ...
      startjoinUnLoginWindow.webContents.openDevTools({
        mode: 'bottom'
    })
    }
    if (startjoinWindow)
  ...
  startunlogin: function(userid, zoomaccesstoken, username, usertoken){
    ...
    //测试 start
    debugger
    var userid = 'C1G-SPzaR_SKrN--7GtQpg'
    var usertoken = 'oegGy7PBTtwz2O6a4oFG3yde6iDo-d8_g2flUgwTYBI.BgUgb1cxY0E1ZWc2cUxjdjBDaWFQVXVjY205TDBjNnExM0NAYmYwNjkwZjMzMzNmZDNhOTEyZTE0OGY4NWUzNGQ5YjFjMzZjZjc0ZDg0YjdlOGU0MDBiMmUxNTZmMjE2MDk2ZQAMM0NCQXVvaVlTM3M9AAA'
    var username = '';
    var zoomaccesstoken = '';
    //测试 end
    ...
    var ret = zoommeeting.StartMeetingWithOutLogin(opt);
